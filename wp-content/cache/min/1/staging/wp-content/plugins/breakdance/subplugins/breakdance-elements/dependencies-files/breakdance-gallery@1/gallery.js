(function(){const{mergeObjects,debounce,matchAnyMedia,matchMedia}=BreakdanceFrontend.utils;const swiperInstance=window.BreakdanceSwiper();class BreakdanceGallery{defaultOptions={type:"grid",row_height:{number:300},columns:{breakpoint_base:3,breakpoint_phone_portrait:1,},slider:{settings:null,pagination:null,},};items=[];filterActiveClass="is-active";sliderActiveClass="is-active";constructor(selector,options){this.relayout=this.relayout.bind(this);this.resize=this.resize.bind(this);this.update=debounce(this._update,100);this.onImagesLoaded=this.onImagesLoaded.bind(this);this.onFilterChange=this.onFilterChange.bind(this);this.selector=selector;this.options=mergeObjects(this.defaultOptions,options);this.init()}
...
filterSlider(category){if(!this.swipers)return;this.swipers.forEach((swiper)=>{swiper.classList.remove(this.sliderActiveClass)});document.querySelector(`[data-gallery-swiper-category="${category}"]`).classList.add(this.sliderActiveClass)}
...
onFilterChange(event){event.preventDefault();const target=event.currentTarget;this.filterBy(target);if(this.options.type!=="slider"){this.maybeJustifyItems()}}
getAll(selector){return[].slice.call(this.element.querySelectorAll(selector))}
initFilters(){const defaultActiveIndex=0;const mobileFilter=this.element.querySelector(".js-tab-select");this.filterButtons=this.getAll(".js-tab");this.filterButtons.forEach((button,index)=>{if(index===defaultActiveIndex){this.filterBy(button)}
...
unjustifyItems(){const items=this.iso.filteredItems;items.forEach((item,index)=>{item.element.style.removeProperty("width");item.element.style.removeProperty("height")})}
getMasonryGap(){if(!this.options.gap)return 0;const{BASE_BREAKPOINT_ID,breakpoints}=window.BreakdanceFrontend.data;const foundBreakpoint=breakpoints.filter(b=>b.id!==BASE_BREAKPOINT_ID).find(b=>matchMedia(b.id));const currentBreakpoint=foundBreakpoint?.id||BASE_BREAKPOINT_ID;const gap=this.options.gap[currentBreakpoint]?.number||this.options.gap.number||0;return Math.max(gap,0)}
...
resize(){this.maybeJustifyItems()}
destroy(){this.destroyIsotope();this.destroySwiper();removeEventListener("resize",this.resize)}
...
initIsotope(layoutInstant){const config=this.getIsotopeSettings(layoutInstant);this.iso=new Isotope(this.galleryEl,config);this.appendItems(this.items);this.relayout();if(this.checkForImagesLoaded()){imagesLoaded(this.element).on("progress",this.onImagesLoaded)}}
initSwiper(){this.swipers=this.element.querySelectorAll(".ee-gallery-swiper");if(!this.swipers)return;this.swipers.forEach((swiper)=>{swiperInstance.update({id:swiper.id,selector:`#${swiper.id}`,settings:this.options.slider.settings,paginationSettings:this.options.slider.pagination,})})}
destroySwiper(){if(!this.swipers)return;this.swipers.forEach((swiper)=>{swiperInstance.destroy(swiper.id)})}
init(layoutInstant){this.element=document.querySelector(this.selector);if(this.options.type==="slider"){this.initSwiper()}else{this.galleryEl=this.element.querySelector(".ee-gallery");this.items=this.getItemElems(this.element);this.initIsotope(layoutInstant);this.maybeJustifyItems()}
this.initFilters();addEventListener("resize",this.resize)}}
