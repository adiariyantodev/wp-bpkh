...
return typeof this.selector==="string"?this.selector.split("-").pop():"default"}
getLibOptions(){const settings=this.options;const namespace=this.getNamespace();const className=`breakdance-gallery-${namespace}`;return{plugins:[lgAutoplay,lgFullscreen,lgZoom,lgVideo,lgThumbnail],licenseKey:"9E9C183D-14564BA5-B09B8BE0-F8B54A20",addClass:className,autoplay:settings.autoplay,slideShowAutoplay:settings.autoplay,slideShowInterval:settings.speed.number,autoplayVideoOnSlide:settings.autoplay_videos,thumbnail:settings.thumbnails,animateThumb:settings.animated_thumbnails,zoomFromOrigin:settings.animated_thumbnails,zoom:!0,download:!1,}}
...
initDynamic(){const items=this.getItems();this.element.addEventListener("pointerup",()=>{this.createGallery({dynamic:!0,dynamicEl:items,});setTimeout(()=>this.lightbox.openGallery(),5)})}
getItemFromAnchor(el){const src=el.href;return this.isVideo(src)?this.getVideoObject(src):{src}}
initFromDOM(){const onClick=(event)=>{const isLinkALightbox=event.currentTarget.dataset.type==="lightbox";if(this.options.watchAttrs&&!isLinkALightbox)return;const item=this.getItemFromAnchor(this.element);if(this.lightbox){this.lightbox.refresh([item])}else{this.createGallery({dynamic:!0,dynamicEl:[item]})}
this.lightbox.openGallery()};this.element.addEventListener("pointerup",onClick)}
initDefault(){this.createGallery({selector:this.options.itemSelector,})}
...
static autoload(element){const selector=".breakdance-link[data-type='lightbox']:not(.is-lightbox-active)";const links=document.querySelectorAll(selector);links.forEach((link)=>{new this(link,{type:"single",watchAttrs:!0,})})}}
